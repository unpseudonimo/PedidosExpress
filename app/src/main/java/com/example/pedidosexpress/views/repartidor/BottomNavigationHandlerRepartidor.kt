...
class BottomNavigationHandlerRepartidor(private val activity: AppCompatActivity) {
    ...
    private val sharedPreferences =
        activity.getSharedPreferences("bottom_nav", AppCompatActivity.MODE_PRIVATE)
    ...
    private fun handleNavigation(item: MenuItem): Boolean {
        ...
        when (itemId) {
            R.id.inicio_item -> {
                ...
                val intent = Intent(activity, HomeRepartidor::class.java)
                activity.finish()
                startActivity(intent)
            }
            R.id.entregas_item -> {
                val intent = Intent(activity, HistorialEntregas::class.java)
                activity.finish()
                startActivity(intent)
            }
            R.id.cuenta_item -> {
                ...
                if (username.isEmpty()) {
                    ...
                    Toast.makeText(activity.applicationContext, "Inicia sesión para hacer entregas", Toast.LENGTH_SHORT).show()
                    val intent = Intent(activity, MainActivity::class.java)
                    activity.finish()
                    startActivity(intent)
                } else {
                    // Usuario logeado, permite acceder a la actividad CuentaConsumidor
                    val intent = Intent(activity, CuentaRepartidor::class.java)
                    activity.finish()
                    startActivity(intent)
                }
                return true
    ...
    companion object {
        private var instance: BottomNavigationHandlerRepartidor? = null

        fun getInstance(activity: Activity): BottomNavigationHandlerRepartidor {
            if (instance == null) {
                instance = BottomNavigationHandlerRepartidor(activity as AppCompatActivity)
            }
            return instance!!
        }
